...
function init() {
  var data;
  var myUrl="jainDataBase/shastra/index.txt";
  var curType;
  $.ajax({
    url:myUrl,
    async: false,
    success: function(data)
    {
      ...
      for(var i=0; i<allItems.categories.length; i++) {
        ...
        var myUrl1="jainDataBase/shastra/"+curType+"/index.txt";
        $.ajax({
          url:myUrl1,
          async: false,
...
function addType() 
{
  ...
  for(i=0; i<allItems.categories.length; i++) {
    ...
    for(j=1; j<allItems[curType]+1; j++) {
      ...
      document.write("<li><a href=\"jainDataBase/shastra/"+curType+"/"+obj.name+"/html/0000_introduction.html\"><font size=4>"+myName+"</font></a></li>");
    }
